...
def verify_user():
    ...
    response = requests.get(f"{urls}/checkuser/:sessionId", headers=headers)
...
async def api_index() -> str:
    result = verify_user()
    if not isinstance(result, AuthUser):
        return result
    """Main page"""
...
@app.route("/generateWakewordPMDLFile", methods=["POST"])
async def api_generate() -> Response:
    ...
    wake_word = request.args.get("wakeWord") or form.get("wakeWord")
    ...
    auth_header = request.headers.get("Authorization")
    scheme, token = auth_header.split()
    decoded = jwt.decode(token, key=os.getenv("JWT_SECRET"), verify=True,algorithms="HS512")
    key_name = f"{decoded['_id']}/voice_sample/{wake_word}/{wake_word}.pmdl"
    s3_client.upload_file(str(model_path), "userdocbucket", key_name)
    uri = f"https://userdocbucket.s3.ap-south-1.amazonaws.com/{decoded['_id']}/voice_sample/{wake_word}/{wake_word}.pmdl"
    urls = os.getenv("URL")
    requests.post(f"{urls}/setProfile/:sessionId", headers={"Authorization": f"Bearer {token}"}, json={"wakeword_pmdl": uri})
